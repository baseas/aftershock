...
bot_character_t *BotLoadCharacterFromFile(char *charfile, int skill)
{
	...
	while(PC_ReadToken(source, &token))
	{
		if (!strcmp(token.string, "skill"))
		{
			...
			// parse all skill values below the desired skill,
			// so the bot files can be smaller
...
int BotLoadCachedCharacter(char *charfile, float skill, int reload)
{
	...
	if (!reload)
	{
		...
		if (cachedhandle)
		{
			botimport.Print(PRT_DEBUG, "loaded cached skill %f from %s\n", skill, charfile);
			return cachedhandle;
		} //end if
	} //end else
	//
	...
	if (ch)
	{
		...
		//
		botimport.Print(PRT_DEBUG, "loaded skill %d from %s\n", intskill, charfile);
#ifdef DEBUG
		if (botDeveloper)
		{
			botimport.Print(PRT_DEBUG, "skill %d loaded in %d msec from %s\n", intskill, Sys_MilliSeconds() - starttime, charfile);
		} //end if
#endif //DEBUG
		return handle;
	} //end if
	...
	if (!reload)
	{
		...
		if (cachedhandle)
		{
			botimport.Print(PRT_DEBUG, "loaded cached default skill %d from %s\n", intskill, charfile);
			return cachedhandle;
		} //end if
	} //end if
	...
	{
		botcharacters[handle] = ch;
		botimport.Print(PRT_DEBUG, "loaded default skill %d from %s\n", intskill, charfile);
		return handle;
	} //end if
	//
	if (!reload)
	...
	{
		botcharacters[handle] = ch;
		botimport.Print(PRT_DEBUG, "loaded skill %f from %s\n", ch->skill, charfile);
		return handle;
	} //end if
	//
	if (!reload)
	{
		...
		if (cachedhandle)
		{
			botimport.Print(PRT_DEBUG, "loaded cached default skill %f from %s\n", botcharacters[cachedhandle]->skill, charfile);
			return cachedhandle;
		} //end if
	} //end if
	//try to load a character with any skill
	...
	{
		botcharacters[handle] = ch;
		botimport.Print(PRT_DEBUG, "loaded default skill %f from %s\n", ch->skill, charfile);
		return handle;
	} //end if
	//
