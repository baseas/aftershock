...
static int IsUnacceptableError(playerState_t *ps, playerState_t *pps)
{
	...
	if (pps->weaponTime != ps->weaponTime || pps->gravity != ps->gravity
		|| pps->speed != ps->speed || pps->groundEntityNum != ps->groundEntityNum
...
void CG_PredictPlayerState(void)
{
	...
	if (cg_nopredict.integer || (cgs.pauseStart && cg.totalTime >= cgs.pauseStart)) {
	...
	if (cgs.gametype == GT_DEFRAG) {
		cg_pmove.tracemask = MASK_SOLID;
	}
	else if (cg_pmove.ps->pm_type == PM_DEAD) {
